...
import { clearVocalSpeech } from './speech';
import { createAudioGraph } from './synths';

export function createPlayer(music, lyrics, setCurrentTime, onDone) {
    resetTransport(music.tempo);
    const audioGraph = createAudioGraph(music);
    const parts = createSongParts(audioGraph, music, lyrics);
...
export function disposePlayer(player) {
    ...
    clearVocalSpeech(player.vocalSpeechTimers);
