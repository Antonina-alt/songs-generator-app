...
function generateSongTitle(localeData, region, random) {
    return shouldUseFakerMusic(localeData, 'songTitle') ? generateFakerSongTitle(region, random) : generatePatternTitle(localeData, region, random);
}

function generateAlbumTitle(localeData, region, random) {
    if (isSingle(random)) return getSingleLiteral(localeData);
    return shouldUseFakerMusic(localeData, 'albumTitle') ? generateFakerAlbum(region, random) : generatePatternAlbum(localeData, region, random);
}

function generateArtist(localeData, region, random) {
    return shouldUseFakerMusic(localeData, 'artist') ? generateFakerArtist(region, random) : generateCustomArtist(localeData, region, random);
}

function generateGenre(localeData, region, random) {
    if (shouldUseFakerMusic(localeData, 'genre')) return normalizeFakerText(getSeededFaker(region, random).music.genre());
