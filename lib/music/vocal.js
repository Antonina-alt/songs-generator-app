...
function createVocalPhrase(context) {
    ...
    return createPhraseEvents({ ...context, words, slots: createWordSlots(context.line.time, context.lineDuration, words) });
}
...
function createWordSlots(lineTime, lineDuration, words) {
    const weights = words.map(getWordWeight);
    return words.map((_, index) => createWordSlot({ lineTime, lineDuration, weights, index, isLast: index === words.length - 1 }));
}
...
function getWordWeight(word) {
    ...
    return 0.8 + Math.min(cleanWord.length * 0.16, 2.2) + Math.min(countVowelGroups(cleanWord) * 0.28, 1.1);
}

function countVowelGroups(word) {
    return word.match(/[aeiouyаеёиоуыэюя]+/giu)?.length ?? 1;
}
