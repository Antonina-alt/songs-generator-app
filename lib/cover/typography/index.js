...
export function renderTypography(context) {
    ...
    return getLayoutRenderer(layoutId)({ ...context, layout: { ...context.layout, id: layoutId } });
}
...
function renderCentered(context) {
    const layout = createCenteredTextLayout(context);
    return [`<circle cx="300" cy="300" r="178" fill="${context.palette.background}" opacity="0.64" />`, renderStackedText(context, layout)].join('');
}

function renderCircleLabel(context) {
    const layout = createCircleLabelTextLayout(context);
    return [`<circle cx="300" cy="300" r="138" fill="${context.palette.background}" stroke="${context.palette.text}" stroke-width="3" opacity="0.78" />`, renderStackedText(context, layout)].join('');
}
...
function renderPanelLayout(context, config) {
    const layout = createPanelTextLayout(context, config);
    const layers = [renderPanelBackground(context, config, layout), renderTitleText(context, layout.title), renderArtistText(context, layout.artist)];
...
function createPanelLayoutParts(config, titleBlock, artistBlock) {
    const title = createPanelTitlePart(config, titleBlock);
    return { title, artist: createPanelArtistPart(config, title, artistBlock) };
}

function createPanelTitlePart(config, block) {
    return { block, x: config.box.x + config.box.paddingX, y: config.box.y + config.box.paddingTop, ...config.titleText };
}

function createPanelArtistPart(config, title, block) {
    return { block, x: title.x + (config.artistOffsetX ?? 0), y: title.y + title.block.height + config.box.gap, ...config.artistText };
}
...
function renderPanelBackground(context, config, layout) {
    ...
    return renderRoundedRect(config.box, context.palette.background, 0.72);
}
...
function renderTitleText(context, part) {
    return renderTextBlock(context, part, { weight: part.weight ?? 900 });
}

function renderArtistText(context, part) {
    return renderTextBlock(context, part, { weight: part.weight ?? 600, opacity: part.opacity ?? 0.88 });
}
...
function createTextProps(context, part, defaults) {
    return { x: part.x, y: part.y, fontSize: part.block.fontSize, color: context.palette.text, letterSpacing: context.typography.letterSpacing, ...defaults, ...part.props };
}
...
function createBoxConfig(box, titleFit, artistFit, options = {}) {
    return { box, titleFit: normalizeTitleFit(box, titleFit), artistFit, ...createTextOptions(options), background: options.background ?? createPanelBackground(options.panelOpacity), wrap: options.wrap, artistOffsetX: options.artistOffsetX };
}
...
function createPanelBackground(opacity = 0.72) {
    return (context, config) => renderRoundedRect(config.box, context.palette.background, opacity);
}
...
function createDiagonalRenderOptions() {
    return { titleWeight: 900, artistWeight: 600, background: renderDiagonalBandPanel, wrap: (content) => wrapSvgGroup(content, 'rotate(-11 300 300)'), artistOffsetX: 2 };
}
...
function renderSplitPosterPanel(context, config) {
    return `<rect x="${config.box.x}" y="0" width="${600 - config.box.x}" height="600" fill="${context.palette.background}" opacity="0.76" />`;
}

function renderFloatingCardsPanel(context, config) {
    return [renderFloatingCardShadow(context), renderFloatingCardAccent(context), renderRoundedRect(config.box, context.palette.background, 0.78)].join('');
}

function renderFloatingCardShadow(context) {
    return `<rect x="80" y="112" width="390" height="250" rx="28" fill="${context.palette.background}" opacity="0.62" transform="rotate(-7 300 255)" />`;
}

function renderFloatingCardAccent(context) {
    return `<rect x="126" y="220" width="390" height="210" rx="24" fill="${context.palette.primary}" opacity="0.58" transform="rotate(8 320 320)" />`;
}
...
function renderDiagonalBandPanel(context, config, layout) {
    ...
    return `<rect x="-60" y="240" width="720" height="${height}" fill="${context.palette.primary}" opacity="0.88" />`;
}
