...
export function renderMotif(context) {
    ...
    return renderGenericMotif(context);
}

function renderVinyl({ palette, motifSettings, random }) {
    ...
    rings.push(`<circle cx="${cx}" cy="${cy}" r="${radius}" fill="${palette.primary}" opacity="0.92" />`);

    for (let index = 0; index < ringCount; index++) {
        const ringRadius = 42 + index * ((radius - 48) / ringCount);
        rings.push(`<circle cx="${cx}" cy="${cy}" r="${ringRadius}" fill="none" stroke="${palette.text}" stroke-width="1.5" opacity="0.18" />`);
    }

    rings.push(`<circle cx="${cx}" cy="${cy}" r="${randomIntFromRange(motifSettings.centerRadiusRange, random)}" fill="${palette.secondary}" opacity="0.94" />`);
    rings.push(`<circle cx="${cx}" cy="${cy}" r="14" fill="${palette.background}" />`);
...
function renderEqualizer({ width, height, palette, motifSettings, random }) {
    ...
    for (let index = 0; index < count; index++) {
        ...
        const color = index % 3 === 0 ? palette.accent : palette.secondary;

        bars.push(`<rect x="${x.toFixed(1)}" y="${y}" width="${barWidth.toFixed(1)}" height="${barHeight}" rx="3" fill="${color}" opacity="0.68" />`);
    }

    return bars.join('');
}

function renderWaveform({ width, height, palette, motifSettings, random }) {
    ...
    return `<polyline points="${points.join(' ')}" fill="none" stroke="${palette.accent}" stroke-width="${randomIntFromRange(motifSettings.strokeWidthRange, random)}" stroke-linecap="round" stroke-linejoin="round" opacity="0.82" />`;
}
...
function renderGenericMotif({ width, height, palette, random }) {
    const shapes = Array.from({ length: randomInt(8, 18, random) }, () =>
        renderGenericShape({ width, height, palette, random })
    );

    return shapes.join('');
}

function renderGenericShape({ width, height, palette, random }) {
